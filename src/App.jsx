...
function Home({ navigate, openHelp }) {
  return <main className="home"><section className="hero-section"><div className="hero-brand" aria-label="FeedON"><span>Feed</span><strong>ON</strong><small>학생 주도성을 켜는 퍼스널 피드백 도우미</small></div><p className="eyebrow">PERSONAL FEEDBACK FOR TEACHERS</p><h1>학생의 오늘을 읽고,<br/><em>다음 배움</em>을 함께 엽니다.</h1><p className="lead">수업에서 관찰한 말과 행동을 바탕으로 학생에게 필요한 피드백 방식과 자연스러운 Teacher Talk을 설계하세요.</p><div className="how-it-works"><span><b>1</b> 수행 모습 입력</span><i>→</i><span><b>2</b> 피드백 전략 확인</span><i>→</i><span><b>3</b> Teacher Talk 선택</span></div><div className="track-grid single"><button className="track feedback" onClick={() => navigate('feedback')}><span className="track-num">START</span><span><b>학생 수행 기록하고 피드백 만들기</b><small>필수 입력은 학습 목표와 실제 수행 모습 두 가지예요</small></span><i>→</i></button></div><button className="text-button" onClick={() => openHelp('stages')}><span className="steps-icon">5</span> 피드백 5단계 먼저 알아보기 <span>→</span></button></section><section className="home-note"><span>FeedON의 약속</span><p>학생을 수준으로 나누지 않습니다. 실제 수행의 증거에서 출발해, 학생이 다음 행동을 스스로 선택하도록 돕습니다.</p></section></main>
}
...
function FeedbackForm({ initial, onSubmit, back }) {
  ...
  return <main className="page"><button className="back" onClick={back}>← 처음으로</button><PageIntro step="피드백 만들기 · 1/2" title="학생의 수행을 들려주세요" desc="필수 입력은 학습 목표와 실제 수행 모습 두 가지입니다. 나머지는 피드백의 맥락을 돕는 선택 정보예요."/><form className="form-sheet" onSubmit={submit} noValidate><div className="form-grid"><Field label="학년"><select value={f.grade} onChange={e=>set('grade',e.target.value)}><option value="">선택하지 않아도 돼요</option>{[1,2,3,4,5,6].map(n=><option key={n}>{n}학년</option>)}</select></Field><Field label="과목"><select value={f.subject} onChange={e=>set('subject',e.target.value)}><option value="">선택하지 않아도 돼요</option>{['국어','수학','사회','과학','도덕','체육','음악','미술','실과','영어'].map(x=><option key={x}>{x}</option>)}</select></Field></div><div className="form-grid"><Field label="단원"><input value={f.unit} onChange={e=>set('unit',e.target.value)} placeholder="선택 · 예: 식물의 구조와 기능"/></Field><Field label="차시"><input value={f.session} onChange={e=>set('session',e.target.value)} placeholder="선택 · 예: 3차시"/></Field></div><div className={errors.goal?'field-error':''}><Field label="학습 목표 또는 평가 요소" required><input value={f.goal} onChange={e=>set('goal',e.target.value)} aria-invalid={!!errors.goal} placeholder="예: 관찰 결과를 근거로 예상과 다른 까닭 설명하기"/>{errors.goal&&<span className="error-message" role="alert">{errors.goal}</span>}</Field></div><div className={errors.observation?'field-error':''}><Field label="학생이 보인 실제 수행 모습" required hint="좋은 기록: 예상은 정확히 말했지만 결과를 기록할 때 측정 단위를 두 번 빠뜨렸다. 친구의 기록을 보고 스스로 한 곳을 고쳤다."><textarea rows="7" value={f.observation} onChange={e=>set('observation',e.target.value)} aria-invalid={!!errors.observation} placeholder="‘잘함·부족함’ 같은 판단보다 학생이 실제로 말하거나 행동한 장면을 적어 주세요."/>{errors.observation&&<span className="error-message" role="alert">{errors.observation}</span>}</Field></div><div className="observation-guide"><b>이런 내용을 적으면 좋아요</b><span>학생이 한 말</span><span>시도한 방법</span><span>오류와 수정</span><span>친구와의 상호작용</span></div><div className="form-actions"><button type="button" className="secondary" onClick={back}>취소</button><button className="primary">피드백 제안 확인하기 <span>→</span></button></div></form></main>
}

function buildPersonalizedStages(data) {
  const evidence=data.observation.trim().replace(/\s+/g,' ').slice(0,64)+(data.observation.trim().length>64?'…':'')
  const goal=data.goal.trim()
  const talks=[
    {talk:`${evidence}라는 수행이 확인됐어.`,focus:'수행이 확인됐어'},
    {talk:`${evidence}까지 해냈어. ‘${goal}’과 연결되는 부분이야.`,focus:'까지 해냈어'},
    {talk:`${evidence}까지 확인했어. 이제 ‘${goal}’에 필요한 부분을 한 가지 더 보완해 보자.`,focus:'한 가지 더 보완'},
    {talk:`네 수행을 ‘${goal}’과 비교하면, 다음에는 어떤 부분부터 바꾸면 좋을까?`,focus:'어떤 부분부터 바꾸면 좋을까'},
    {talk:`‘${goal}’을 잘했다고 판단하려면 어떤 기준이 필요할까? 그 기준으로 네 수행을 다시 살펴보자.`,focus:'어떤 기준이 필요할까'},
  ]
...
function FeedbackResult({ data, edit }) {
  ...
  return <main className="page result-page"><button className="back" onClick={edit}>← 수행 기록 수정</button><PageIntro step="피드백 만들기 · 2/2" title="학생의 다음 행동을 위한 피드백" desc={context}/><div className="record-warning"><b>공식 성적·평가 기록이 아닌 피드백 설계 제안입니다.</b><p>교사가 실제 수행 증거와 학습 목표를 다시 확인하고, 학생에게 맞게 수정한 뒤 사용해 주세요.</p></div><div className="result-doc"><Section no="01" title="관찰 사실"><div className="evidence"><span className="result-label fact">교사가 입력한 원문</span><p>“{data.observation}”</p><dl><div><dt>학습 목표</dt><dd>{data.goal}</dd></div></dl></div></Section><Section no="02" title="FeedON 제안"><div className="strategy"><div><span>추천 단계</span><h3>{recommended.name} · {recommended.ko}</h3></div><p>{recommended.desc} {coachReady?'학생이 스스로 확인하거나 수정한 흔적이 있어, 질문을 통해 다음 해결 방법을 이끌어내는 접근을 우선 제안합니다.':'현재 기록만으로 학생의 해결 전략이 충분히 드러나지 않아, 목표와 현재 수행의 차이를 구체적으로 안내하는 접근을 우선 제안합니다.'}</p></div><p className="proposal-note">이 추천은 입력 문장의 표현을 바탕으로 한 로컬 규칙 기반 제안이며, 학생의 성취수준 판정이 아닙니다.</p><h3 className="subhead">추천 피드백 도구</h3><div className="recommended-tools">{recommendedTools.map(t=><ToolCard key={t[1]} tool={t}/>)}</div></Section><Section no="03" title="5단계 Teacher Talk"><p className="section-desc">같은 수행 증거를 5단계의 서로 다른 초점으로 표현했습니다. 학생에게 필요한 단계 하나를 골라 자연스럽게 다듬어 사용하세요.</p><div className="talk-list">{personalizedStages.map((s,i)=>{const isOpen=open.includes(i);return <article key={s.name} className={isOpen?'open':''}><div className="talk-top"><span>0{i+1}</span><div><h3>{s.name} <small>{s.ko}</small></h3><CenterBadge center={s.center}/><blockquote>“<HighlightTalk stage={s}/>”</blockquote></div></div><button className="expand" onClick={()=>setOpen(isOpen?open.filter(x=>x!==i):[...open,i])} aria-expanded={isOpen}>{isOpen?'접기':'단계 설명 보기'} <span>{isOpen?'−':'+'}</span></button>{isOpen&&<div className="talk-detail"><dl><div><dt>단계의 목적</dt><dd>{s.desc}</dd></div><div><dt>사용 시점</dt><dd>{s.when}</dd></div><div><dt>교사가 주의할 점</dt><dd>{s.caution}</dd></div></dl></div>}</article>})}</div></Section><Section no="04" title="교사 확인"><div className="teacher-check"><p>학생에게 말하기 전에 세 가지를 확인해 주세요.</p><label><input type="checkbox"/> 입력한 관찰 사실과 제안 문장이 일치하나요?</label><label><input type="checkbox"/> 학생의 수준을 낙인찍는 표현이 없나요?</label><label><input type="checkbox"/> 학생이 직접 해 볼 다음 행동이 분명한가요?</label></div></Section></div></main>
}
